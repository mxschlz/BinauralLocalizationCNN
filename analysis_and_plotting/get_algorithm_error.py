...
for azim in range(0, 360, 30):
    ...
    azim_binned = azim / 5
    soruce_files = glob(
        '/om/user/francl/recorded_binaural_audio_4078_main_2_mic_array_rescaled/*_{}_azim.wav'.format(azim))
...
def make_azim_vs_folded_rms_plot(df_rms):
    ...
    if 'azim' in df_rms:
        ...
        df_rms.columns = ['Azimuth (Degrees)', 'DOA Algorithm',
                          ...
                          'Architecture Index']
    sns.lineplot(x="Azimuth (Degrees)", y="Folded RMS (Degrees)", hue="DOA Algorithm", style="DOA Algorithm",
                 data=df_rms)
...
def fold_pd_error_data_for_plotting(df_rms, algo_names=['SRP', 'MUSIC', 'TOPS', 'CSSM', 'WAVES']):
    if 'azim' in df_rms:
        ...
        df_rms['rms_folded'] = df_rms['rms_folded'].map(lambda x: x * 5)
    df = pd.DataFrame(
        columns=['Azimuth (Degrees)', 'DOA Algorithm', 'Folded RMS (Degrees)', 'RMS (Degrees)', 'Architecture Index'])
...
def make_azim_vs_folded_rms_plot_all_algorithms(df_rms):
    sns.lineplot(x="Azimuth (Degrees)", y="Folded RMS (Degrees)", ci=68, err_style='bars', hue="DOA Algorithm",
                 data=df_rms)
...
def fig_2f():
    ...
    pd_dataframe_folded = make_dataframe(
        "/om2/user/francl/grp-om2/gahlm/dataset_pipeline_test/arch_number_*[3,4,5,6,7,8,9][0-9]_init_0/batch_conditional_stimRecords_binaural_recorded_testset_4078_main_kemar_0elev_speech_label_iter200000.npy",
        fold_data=True)
    pd_dataframe_folded_2_mic = make_dataframe(
        "/om2/user/francl/new_task_archs/new_task_archs_2_mic_with_reverberation_background_noise_5dB40dBSNR_training/arch_number_*[3,4,5,6,7,8,9][0-9]_init_0/batch_conditional_stimRecords_binaural_recorded_testset_4078_main_2_mic_0elev_speech_label_iter200000.npy",
        fold_data=True)
...
def fig_8f():
    pd_dataframe_folded_normal = make_dataframe(("/om2/user/francl/grp-om2/gahlm/dataset_pipeline_test/"
                                                 ...
                                                 "_testset_4078_main_kemar_0elev_speech_label_iter100000.npy"),
                                                fold_data=True)
    ...
    pd_dataframe_folded_unnatural_sounds = make_dataframe(("/om2/user/francl/new_task_archs/"
                                                           ...
                                                           "arch_number_*[0,1,3,4,5,6,7,8,9][0-9]_init_0/batch_conditional_stimRecords_binaural_recorded"
                                                           "_testset_4078_main_kemar_0elev_speech_label_iter150000.npy"),
                                                          fold_data=True)
