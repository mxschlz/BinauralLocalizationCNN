...
def test_erb_filters(rfn, mode='all', verbose=0):
    ...
    try:
        for i in range(len(sample_factor_list)):
            ...
            if mode == 'all' or mode == 'matlab':
                ...
                assert np.allclose(hz_cutoffs,
                                   mlab[hz_cutoffs_key]), 'Hz_cutoff mismatch: %s(...)' % matlab_api_fx.__name__
                if verbose > 0:
            ...
            if mode == 'all' or mode == 'literal':
                ...
                if sample_factor == 1:
                    ...
                    assert np.allclose(hz_cutoffs,
                                       mlab[hz_cutoffs_key]), 'Hz_cutoff mismatch: make_erb_cos_filters(...) (literal)'
                    if verbose > 0:
        ...
        print('<Input Params: signal_length: %s, sr: %s, N: %s, low_lim: %s, hi_lim: %s, sample_factor: %s>' %
              (signal_length, sr, N, low_lim, hi_lim, sample_factor))
        print('filts: %s python: %s, (%s, %s), matlab: %s, (%s, %s)' % (
        np.abs(filts - mlab[filts_key]).max(), filts.shape, filts.min(), filts.max(),
        mlab[filts_key].shape, mlab[filts_key].min(), mlab[filts_key].max()))
        print('freqs: %s python: %s, (%s, %s), matlab: %s, (%s, %s)' % (
        np.abs(freqs - mlab[freqs_key]).max(), freqs.shape, freqs.min(), freqs.max(),
        mlab[freqs_key].shape, mlab[freqs_key].min(), mlab[freqs_key].max()))
        print('hz_cutoffs: %s python: %s, (%s, %s), matlab: %s, (%s, %s)' % (
        np.abs(hz_cutoffs - mlab[hz_cutoffs_key]).max(), hz_cutoffs.shape, hz_cutoffs.min(), hz_cutoffs.max(),
...
def test_run_dir(in_path, verbose=0):
    fntp = [os.path.join(in_path, f) for f in os.listdir(in_path) if
            'erb_human_filters_test' in f and f.endswith('.mat')]
