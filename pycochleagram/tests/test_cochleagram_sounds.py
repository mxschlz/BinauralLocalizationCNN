    ...
    try:
        for i in range(len(sample_factor_list)):
            ...
            if mode == 'all' or mode == 'nx':
                ...
                assert np.allclose(filts, mlab[
                    filts_key].T), 'filts mismatch: make_erb_cos_filters_nx(...)'  # transpose because of python
                ...
                assert np.allclose(subbands_dict['fft_sample'],
                                   mlab['fft_sample_1'].flatten()), 'fft sample mismatch: make_erb_cos_filters_nx(...)'
                assert np.allclose(subbands_dict['subbands'],
                                   mlab[subbands_key].T), 'subband mismatch: make_erb_cos_filters_nx(...)'
                if verbose > 0:
        ...
        for i in range(len(sample_factor_list)):
            ...
            if erb_filter_mode == 'all' or erb_filter_mode == 'nx':
                if coch_mode == 'coch':
                    ...
                    coch = cgram.human_cochleagram(signal, sr, N, low_lim, hi_lim, sample_factor, pad_factor=1,
                                                   strict=False)
                    assert np.allclose(coch, mlab[
                        sub_envs_key].T), 'subband_env mismatch: make_erb_cos_filters_nx(...)'  # transpose for matlab-to-python
                    if verbose > 0:
                    ...
                    assert np.allclose(filts, mlab[
                        filts_key].T), 'filts mismatch: make_erb_cos_filters_nx(...)'  # transpose because of python
                    assert np.allclose(freqs, mlab[freqs_key]), 'Freqs mismatch: make_erb_cos_filters_nx(...)'
                    assert np.allclose(hz_cutoffs,
                                       mlab[hz_cutoffs_key]), 'Hz_cutoff mismatch: make_erb_cos_filters_nx(...)'
                    if verbose > 0:
                        print('\tPASSED ERB Filters: make_erb_cos_filters_nx(%s)' % sample_factor)
                ...
                if coch_mode == 'subband':
                    ...
                    assert np.allclose(subband_envs, mlab[
                        sub_envs_key].T), 'subband_env mismatch: make_erb_cos_filters_nx(...)'  # transpose for matlab-to-python
                    if verbose > 0:
