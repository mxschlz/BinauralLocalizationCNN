...
def wrapped_subband_memmap(signal, out_array, start_idx, end_idx):
    ...
    out_array[start_idx:end_idx] = cgram.human_cochleagram(signal, sr, n=n, low_lim=low_lim, hi_lim=high_lim,
                                                           sample_factor=sample_factor, pad_factor=None,
                                                           downsample=downsample, nonlinearity=nonlinearity,
                                                           fft_mode='np', ret_mode='subband', strict=True).astype(
        np.float32)
    out_array.flush()
...
def run_francl_test(in_dir, n_per_batch=8, num_workers=8):
    ...
    fntp_all = [wav.read(os.path.join(in_dir, f))[1][:30000].swapaxes(1, 0) for f in os.listdir(in_dir) if
                f.endswith('.wav')]
    ...
    with Parallel(n_jobs=num_workers) as parallel:
        ...
        while True:
            ...
            print('memmap cgram out shape ', cgram_shape, '[%s, %s]' % (start_ind, end_ind), subbatch_idx_list,
                  fntp.shape)
            # pdb.set_trace()
            start_time = time.time()
            parallel(delayed(wrapped_subband_memmap)(fntp[start:end], cgram_memmap, start, end) for start, end in
                     subbatch_idx_list)
...
def wrapped_cochleagram(signal, extra_data=[]):
    ...
    coch = cgram.human_cochleagram(signal, sr, n=n, low_lim=low_lim, hi_lim=high_lim,
                                   sample_factor=sample_factor, pad_factor=None, downsample=downsample,
...
def queued_generator_subbatched_helper(output_queue, data_src, callback_fx, num_epochs, batch_size, num_workers,
                                       extra_data_src_list=[]):
    subbatch_size = ceil(batch_size / num_workers)
    ...
    while do_loop:
        for epoch in range(num_epochs):
            for idx in range(num_per_epoch):
                ...
                deferred = [pool.apply(wrapped_cochleagram, (batch_data[start:end],)) for start, end in
                            subbatch_idx_list]
...
def queued_generator_helper(output_queue, data_src, callback_fx, num_epochs, batch_size, num_workers,
                            extra_data_src_list=[], oncomplete_callback=None):
    ...
    for epoch in range(num_epochs):
        ...
        deferred = [pool.apply_async(callback_fx, (data_src[start:end], [x[start:end] for x in extra_data_src_list]),
                                     callback=oncomplete_callback) for start, end in batch_idx_list]
...
def queued_generator(data_src, callback_fx, num_epochs, batch_size, num_workers, extra_data_src_list=[],
    ...
    print('queued_generator --> %s: num_workers: %s batch_size: %s prewait_seconds: %s' % (
    callback_fx.__name__, num_workers, batch_size, queue_prewait_seconds))
    out_queue = Queue()
    p = Process(target=queued_generator_helper,
                args=(out_queue, data_src, callback_fx, num_epochs, batch_size, num_workers, extra_data_src_list))
    try:
        p.start()
        time.sleep(queue_prewait_seconds)
        for i in range(total_batches):
...
def cochleagram_generator(data_src, num_epochs, batch_size, num_workers, extra_data_src_list=[],
                          queue_prewait_seconds=0):
    if num_workers == 1:
        def single_thread_gx():
            ...
            print('single process cgram generator --> %s: num_workers: %s batch_size: %s prewait_seconds: %s' % (
            wrapped_cochleagram.__name__, num_workers, batch_size, queue_prewait_seconds))
            for i in range(total_batches):
                start = i * batch_size
                end = (i + 1) * batch_size
        ...
        cgram_gx = queued_generator(data_src, wrapped_cochleagram_main, num_epochs, batch_size, num_workers=num_workers,
                                    extra_data_src_list=extra_data_src_list,
                                    queue_prewait_seconds=queue_prewait_seconds)
    # ipdb.set_trace()
    return cgram_gx
...
def main():
    ...
    with h5py.File(data_fn, 'r') as h5rf:
        ...
        cgram_gx = cochleagram_generator(signal_data, num_epochs, batch_size, num_workers, extra_data_src_list,
                                         queue_prewait_seconds)
...
def francl_main():
    n_per_batch = 40
    gx = run_francl_test(
        '/Users/raygon/Desktop/mdLab/projects/public/cochleagram/sandbox/francl_testStimOnTheFlyCgramGeneration',
        n_per_batch=n_per_batch, num_workers=8)
    start_time = time.time()
    [next(gx) for x in gx]
