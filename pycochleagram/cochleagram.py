...
def invert_cochleagram_with_filterbank(cochleagram, filters, sr, target_rms=100,
    ...
    if downsample is None or callable(downsample):
        _wrapped_downsample = lambda coch, inv: apply_envelope_downsample(coch, downsample,
                                                                          invert=inv)  # downsample is None or callable
    else:
        # interpret downsample as new sampling rate
...
def invert_cochleagram(cochleagram, sr, n, low_lim, hi_lim, sample_factor,
    ...
    # invert filterbank
    inv_signal, inv_coch = invert_cochleagram_with_filterbank(cochleagram_ref, filts, sr, target_rms=target_rms,
                                                              n_iter=n_iter)
