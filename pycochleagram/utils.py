...
if check_if_display_exists():
    from matplotlib.pyplot import imshow, show, plot, savefig
else:
    import matplotlib
...
def filtshow(freqs, filts, hz_cutoffs=None, full_filter=True, use_log_x=False, interact=True):
    ...
    savefig("/om/user/francl/cochleagram_filters1.png")
    if interact:
        show()
    return f
    ...
    if rescale == 'standardize':
        if issubclass(snd_array.dtype.type, np.integer):
            snd_array = snd_array / float(
                np.iinfo(snd_array.dtype).max)  # rescale so max value allowed by precision has value 1
        elif issubclass(snd_array.dtype.type, np.floating):
            snd_array = snd_array / float(
                np.finfo(snd_array.dtype).max)  # rescale so max value allowed by precision has value 1
        else:
...
def play_array(snd_array, sr=44100, rescale='normalize', pyaudio_params={}, ignore_warning=False):
    ...
    _pyaudio_params = {'format': pyaudio.paFloat32,
                       ...
                       'rate': sr,
                       'frames_per_buffer': 1,
                       # I don't know what this does, but default of 1024 causes issues with TIMIT in py2.7
                       'output': True,
                       'output_device_index': 1}
